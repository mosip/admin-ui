...
export class SingleViewComponent implements OnDestroy {
  ...
  async initializeComponent() {
    ...
    this.activatedRoute.params.subscribe((response) => {
      ...
      this.masterDataName =
        defaultJson.masterdataMapping[response.type].name[this.primaryLangCode];
    });
    this.loadData();
  }
  ...
  setHeaderData() {
    if (this.primaryData) {
      ...
      this.headerData = new HeaderModel(
        ...
        this.primaryData.createdDateTime
          ? this.primaryData.createdDateTime
          : "-",
        this.primaryData.createdBy ? this.primaryData.createdBy : "-",
        this.primaryData.updatedDateTime
          ? this.primaryData.updatedDateTime
          : "-",
  ...
  getData(language: string, isPrimary: boolean) {
    return new Promise((resolve, reject) => {
      ...
      if (this.mapping.apiName !== "dynamicfields") {
        filterModel = new FilterModel(this.mapping.idKey, "equals", this.id);
        this.fetchRequest.filters = [filterModel];
      } else {
        filterModel = new FilterModel("valueJson", "contains", this.id);
        this.fetchRequest.filters = [filterModel];
      }
              ...
              if (response.response.data) {
                ...
                if (isPrimary) {
                  if (this.masterdataType.toLowerCase() === "dynamicfields") {
                    ...
                    this.primaryData.fieldVal = JSON.stringify(
                      this.primaryData.fieldVal
                    );
                  } else {
                    this.primaryData = response.response.data[0];
                  }
