...
export class CreateComponent {
          ...
          .subscribe((month: string) => {
            ...
            this.translateService
              ...
              .subscribe((day: string) => {
                ...
                console.log(
                  `locale for datePicker: ${localeId} : ${this.localeDtFormat}`
                );
              });
          });
      });
  ...
  onCreate() {
    ...
    const zone = this.dropDownValues.zone.primary.filter(
      (z) => z.code === this.primaryForm.controls.zone.value
    );
    data = {
      case: "CONFIRMATION",
      ...
      message:
        ...
        this.popupMessages["create"].message[2],
  ...
  initializePrimaryForm() {
    ...
    this.primaryForm = this.formBuilder.group({
      ...
      noKiosk: [0, [Validators.required, Validators.min(0), ValidateKiosk]],
  ...
  validateAndLoadLunchStartTime() {
    if (
      this.primaryForm.controls.startTime.value !== "" &&
      this.primaryForm.controls.startTime.valid
    ) {
      const x = [...this.allSlots];
      let startIndex = x.indexOf(this.primaryForm.controls.startTime.value) + 1;
      if (
        this.primaryForm.controls.lunchEndTime.value !== "" &&
        this.primaryForm.controls.lunchEndTime.valid
      ) {
  ...
  validateAndLoadLunchEndTime() {
    if (
      this.primaryForm.controls.endTime.value !== "" &&
      this.primaryForm.controls.endTime.valid
    ) {
      const x = [...this.allSlots];
      const endIndex = x.indexOf(this.primaryForm.controls.endTime.value);
      if (
        this.primaryForm.controls.lunchStartTime.value !== "" &&
        this.primaryForm.controls.lunchStartTime.valid
      ) {
        const startIndex =
          x.indexOf(this.primaryForm.controls.lunchStartTime.value) + 1;
        this.dropDownValues.lunchEndTime = x.slice(startIndex, endIndex);
      } else {
        const startIndex =
          x.indexOf(this.primaryForm.controls.startTime.value) + 1;
        this.dropDownValues.lunchEndTime = x.slice(startIndex, endIndex);
      }
    }
    // else {
  ...
  canDeactivate(): Observable<any> | boolean {
    ...
    if (this.primaryForm.touched && !this.createUpdate) {
      return this.dialog
